...
export default function EventFormPage({
  ...
  const fixedMeetingPreview = normalHasFixedMeeting
    ? resolveMeetingLink({ kind: "normal", modality, title })
    : null;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const finalMeeting = resolveMeetingLink({
        kind,
        modality,
        title: finalTitle,
        meetingLinkType: kind === "normal" && customMeetingUrl.trim() ? "custom" : "none",
        meetingUrl: kind === "normal" ? customMeetingUrl : ""
      });
      if (
        kind === "normal" &&
        customMeetingUrl.trim() &&
        finalMeeting.meetingLinkType === "custom" &&
        !finalMeeting.meetingUrl
      ) {
                ...
                <>
                  ...
                  <label className="md:col-span-2">
                    ...
                    <textarea
                      ...
                      placeholder="Escribe los detalles, temas o notas de la reunión."
                      maxLength={5000}
                    />
                  </label>
                  <label className="md:col-span-2">
                    <span className="section-label mb-2 block">
                      Enlace de reunión {normalHasFixedMeeting ? "(fijo)" : "(opcional)"}
                    </span>
                    <input
                      className="input-field"
                      type="url"
                      inputMode="url"
                      value={fixedMeetingPreview?.meetingUrl || customMeetingUrl}
                      onChange={(e) => setCustomMeetingUrl(e.target.value)}
                      readOnly={normalHasFixedMeeting}
